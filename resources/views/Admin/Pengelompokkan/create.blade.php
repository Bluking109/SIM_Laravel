...
@section('title_admin', 'Tambah Pengelompokan')
@section('content')

<body>
    <div class="card-body">

        <div class="col-12 col-md-18"></div>

        @if (session('error'))
        <div class="alert alert-danger">{{ session('error') }}</div>
        @endif

        <form name="formId" id="formId" class="login100-form validate-form p-b-33 p-t-5" method="post" enctype="multipart/form-data" action="{{ route('Admin.Pengelompokkan.store') }}" onsubmit="return validateForm(event)">
            @csrf
            <div class="c`ard mt-5 ">
                <div class="card-header">
                    <label><b>Buat Kelompok</b></label>
                </div>
                    ...
                    <div class="row">
                        <div class="col-lg-6">
                            <div class="form-group">
                                ...
                                <input class="form-control text-box single-line" data-val="true" placeholder="Kelompok X" data-val-required="The pin_password field is required." id="kel_nama" name="kel_nama" type="text">
                                <span id="kelNamaError" style="color: red;"></span>
                            </div>
                        </div>
                        <div class="col-lg-6">
                            <div class="form-group">
                                <label style='font-weight: bold;' for="addAre">Departemen/Area Magang<b style="color: red">*</b></label>
                                <input class="form-control text-box single-line" data-val="true" data-val-required="The pin_password field is required." id="kel_departemen" name="kel_departemen" type="text">
                                <span id="kelDepartemenError" style="color: red;"></span>
                            </div>
                        </div>
                        <div class="col-lg-6">
                            <div class="form-group">
                                <label style='font-weight: bold;' for="addPembimbing">Pembimbing
                                    Industri</label>
                                <select name="pin_id" class="form-control" id="pin_id">
                                    <option value="">-- Pilih Pembimbing --</option>
                                    @foreach ($pembimbing as $pembimbingID => $pin_nama)
                                    <option value="{{ $pembimbingID }}" @selected(old('pin_id')==$pembimbingID)>
                                        {{ $pin_nama }}
                                    </option>
                                    @endforeach
                                </select>
            ...
            <table class="table table-hover table-bordered table-condensed table-striped grid">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><input type="checkbox" class="mhs_id" name="mhs_ids[]" value="{{ $cus->mhs_id }}">
                        </td>
                    </tr>
                    @empty
                    <tr>
